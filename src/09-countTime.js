...
Tienen unas tiras led que son como un Array. Cada posición es un led y puede 
ser estar encendido (1) o apagado (0).
...
Nos han pedido un programa que nos diga cuantos segundos deben pasar hasta que todos 
los leds están encendidos. Los segundos se expresan en un número entero. Por ejemplo:
...
export const countTime = (leds) => {
    if (!Array.isArray(leds)) throw new Error("Invalid Param");
    if (leds.length === 0) throw new Error("Invalid Param");
    if (leds.some(e => typeof e !== "number")) throw new Error("Invalid Param");
    if (leds.some(e => e > 1 || e < 0)) throw new Error("Invalid Param");
