/*
Se han estropeado algunos trineos eléctricos y los elfos están buscando 
piezas de repuesto para arreglarlos, pero no tienen claro si las piezas 
que tienen sirven.
...
Nuestra función debe devolver un booleano que indique si la pieza de repuesto es 
válida o no con esa regla:
...
export const checkPart = (part) => {
    if (typeof part !== "string") throw new Error("Invalid Param");

    return [...part].some((_letter, index) => {
        const nextPart = part.substring(0, index) + part.substring(index + 1);
